...
class TaylorDiagram(object):
    ...
    def __init__(self, refstd, fig=None, rect=111, label='_', **kwargs):
...
if __name__=='__main__':
    ...
    for i,m in enumerate([m1,m2,m3]):
        ax1.plot(x,m, c=pl_colors[i], label='Model %d' % (i+1))
    ax1.legend(numpoints=1, prop=dict(size='small'), loc='best')

    # Add samples to Taylor diagram
    for i,(stddev,corrcoef) in enumerate(samples):
        #dia.add_sample(stddev, corrcoef, marker='s', ls='', c=colors[i],
        dia.add_sample(stddev, corrcoef, marker='s', ls='', c=pl_colors[i],
                       label="Model %d" % (i+1))
